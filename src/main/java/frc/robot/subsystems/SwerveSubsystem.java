...
public class SwerveSubsystem extends SubsystemBase
{
  ...
  private double Targetx = 12, Targety = 1;
  ...
  public AHRS navx;
   ...
   // o kOnboardCS0 si lo conectas diferente
  ...
  public double error() {
    ...
    if (obj != null) {
        ...
        double dx = obj[0] - getPose().getX();
        double dy = obj[1] - getPose().getY();
  ...
  public double[] objetivo (){
    if(DriverStation.getAlliance().isPresent()){
      if(DriverStation.getAlliance().get() == DriverStation.Alliance.Red){
        return new double []{11.915, 4.040};
      }else{
        return new double []{4.625, 4.040};
      }
    }else{
      return null;
    }
  ...
  public double desiredHeadingDeg() {
    ...
    double dy = Targety - getPose().getY();
    ...
    if (targetAngleDeg < 0) targetAngleDeg += 360.0;
  ...
  public void filterOutOfFieldData(){
    Pose2d currentPosition = this.getPose();
          ...
          if (currentPosition.getX() < -Constants.fieldBorderMargin
          || currentPosition.getX()
              > Constants.fieldLength
                  + Constants.fieldBorderMargin
          || currentPosition.getY() < -Constants.fieldBorderMargin
          || currentPosition.getY()
              > Constants.fieldWidth
                  + Constants.fieldBorderMargin) {
                    ...
                    new Pose2d(
                        Math.max(
                            0, Math.min(Constants.fieldLength, currentPosition.getX())),
                        Math.max(
                            0, Math.min(Constants.fieldWidth, currentPosition.getY())),
                        currentPosition.getRotation());
        
                      this.resetOdometry(placeHolderPose);
                    }
  }
    ...
    try
    {
      ...
      AutoBuilder.configure(
          ...
          new PPHolonomicDriveController(
              ...
              new PIDConstants(1.15, 0.0, 2.5),
              // Translation PID constants
              new PIDConstants(0.35, 0.0, 1.75)
...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
  ...
  /**
   * 2 the robot given a chassis field oriented velocity.
   *
